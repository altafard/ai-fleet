...
func newRoot(code *int) *cobra.Command {
	...
	f.StringVar(&o.Dockerfile, "dockerfile", "", "path to the environment Dockerfile (default: the one generated by 'ai-fleet init')")
	...
	f.StringVar(&o.Model, "model", "", "claude model for the run, alias or full ID (required unless configured)")
	f.StringVar(&o.Effort, "effort", "", "claude reasoning effort: low|medium|high|xhigh|max (required unless configured)")
	f.StringVar(&o.GitAuthorName, "git-author-name", "", "git author name for run commits (required unless configured)")
	f.StringVar(&o.GitAuthorEmail, "git-author-email", "", "git author email for run commits (required unless configured)")
